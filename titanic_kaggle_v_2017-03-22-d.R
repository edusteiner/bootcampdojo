...
# Clean Missing Data (column "Age")
  ...
  nrow(titanic.full[titanic.full["Pclass"]==1 & titanic.full["Sex"]== "male" & is.na(titanic.full["Age"]) ,])
  
  nrow(titanic.full[titanic.full["Pclass"]==1 & 
       titanic.full["Sex"]== "male" &
        is.na(titanic.full["Age"]),])
  

  # Relacao Age / TicketPrice em Classe 2 e 3
  xyplot(titanic.full[titanic.full["Pclass"]>1,"Age"] ~ titanic.full[titanic.full["Pclass"]>1,"Fare"])
  
  corrgram(titanic.full[titanic.full["Pclass"]>1,], order=NULL, lower.panel=panel.shade,
           upper.panel=NULL, text.panel=panel.txt,
           main="Correlacao Titanic Train")
    
  age.equation <- "Age ~ Pclass + Sex + Parch + SibSp"
  
  no_age <- titanic.full[titanic.full["Pclass"]==1 & is.na(titanic.full["Age"]) & titanic.full["isTrainSet"]==TRUE ,]
